...
describe 'DB', ->
  describe 'search', ->
    ...
    it 'sort function are caleed', ->
      spyOn(SetIndex::, 'search').andReturn(new Result(['1m', '5m', '3m']))
      spyOn(TrieIndex::, 'search').andReturn(new Result(['3m', '1m', '5m']))
      db = new DB()
      db.createIndex('tags', 'set')
      db.createIndex('name', 'trie')
      ...
      results = db.search({tags: '1t', name: 'KFC'}, (a, b) -> a.value - b.value)
